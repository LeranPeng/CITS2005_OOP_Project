...
/** A {@link Predicate} that requires a {@link Student} to have taken a particular unit. */
class StudentHasMark implements Predicate<Student> {
    String unit;

    public StudentHasMark(String unit) {
        this.unit = unit;
    }
...
public class StudentStats {
    ...
    public static Iterator<Student> unitNewestStudents(StudentList list, String unit) {
        return Itertools.filter(
                Itertools.reversed(new StudentListIterator(list)), new StudentHasMark(unit));
    }
}
